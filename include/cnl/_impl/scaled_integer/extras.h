...
namespace cnl {
    ...
    CNL_NODISCARD constexpr auto abs(scaled_integer<Rep, Scale> const& x) noexcept -> decltype(-x)
    {
    ...
    namespace _impl {
        ...
        {
            return (bit != Rep{0}) ? (n >= result + bit)
                            ? sqrt_solve3<Rep>(
                                      static_cast<Rep>(n - (result + bit)),
                                      static_cast<Rep>(bit >> 2),
                                      static_cast<Rep>((result >> 1) + bit))
                            : sqrt_solve3<Rep>(
                                      n, static_cast<Rep>(bit >> 2), static_cast<Rep>(result >> 1))
                                   : result;
        }
    ...
    CNL_NODISCARD constexpr auto sqrt(scaled_integer<Rep, power<Exponent, Radix>> const& x)
    ...
    CNL_NODISCARD constexpr auto floor(scaled_integer<Rep, power<Exponent, Radix>> const& x)
            -> decltype(_impl::from_rep<scaled_integer<Rep, power<0, Radix>>>(
                    _impl::to_rep(x) >> constant<-Exponent>()))
    {
        static_assert(
                Radix == 2,
                "cnl::floor(scaled_integer<Rep, power<Exponent, Radix>>) not implemented for "
                "Exponent<0 && Radix!=2");
