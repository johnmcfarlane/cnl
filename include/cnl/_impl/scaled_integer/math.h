        ...
        namespace fp {
                    ...
                    scaled_integer<Rep, power<Exponent>> const& x, Rep const& floored)
            {
                return floored <= Exponent
                        ? rep_t<Intermediate>{1} // return immediately if the shift would result in
                                                 // all bits being shifted out
                        // Do the shifts manually. Once the branch with shift operators is merged,
                        // could use those
                        : (_impl::to_rep(exp2m1_0to1<Rep, Exponent>(fractional(
                                   x, floored))) // Calculate the exponent of the fraction part
                           >> (-tag_t<Intermediate>::exponent + Exponent -
                               floored)) // shift it to the right place
                                +
                                (Rep{1} << (floored - Exponent)); // The constant term must be one,
                                                                  // to make integer powers correct
            }
        }
    }
