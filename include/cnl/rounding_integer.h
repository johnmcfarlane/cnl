...
namespace cnl {
    ...
    struct rounding<Number, _impl::enable_if_t<is_composite<Number>::value&&!_impl::is_number<Number>::value>>
            : rounding<_impl::to_rep_t<Number>> {
    };

    template<typename Rep, class Tag>
    struct rounding<_impl::number<Rep, Tag>, _impl::enable_if_t<!_impl::is_rounding_tag<Tag>::value>>
            : rounding<Rep> {
    };
