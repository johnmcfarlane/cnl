...
namespace {
    TEST(static_number, to_chars)  // NOLINT
    {
        ...
        auto result =
                cnl::to_chars(chars_first, chars_last, cnl::static_number<24, cnl::power<-8>>{-45678.765625});
    ...
    TEST(static_number, to_chars_wider)  // NOLINT
    {
        ...
        auto* chars_last = chars_first + num_chars;
        auto result =
                cnl::to_chars(chars_first, chars_last, cnl::static_number<28, cnl::power<-8>>{937016.765625});
        ASSERT_EQ(std::errc{}, result.ec);
        ASSERT_EQ(chars_last, result.ptr);
        *chars_last = '\0';
