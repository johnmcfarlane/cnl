...
{ // NOLINT
...
#if (FPTESTEXP < 0)
    for (int i = std::max(
                 -cnl::_impl::fractional_digits<fp>::value,
                 -(cnl::_impl::scale<cnl::_impl::integer_digits<fp>::value, 2, int32_t>(1)) + 1);
         i < std::min(0, cnl::_impl::integer_digits<fp>::value - 1); i++) {
        fp lhs{exp2(fp{static_cast<double>(i)})};
        EXPECT_EQ(lhs, cnl::_impl::from_rep<fp>(1 << (-FPTESTEXP + i)))
                << "i = " << i << ", fixed point raw: " << cnl::_impl::to_rep(lhs)
                << " should be: " << (1 << (-FPTESTEXP + i));
    }
#endif
    ...
    for (int i = -cnl::_impl::fractional_digits<fp>::value;
         i < cnl::_impl::integer_digits<fp>::value; i++) {
    ...
    auto minimum = cnl::_impl::from_rep<fp>(
            cnl::_impl::to_rep(fp{static_cast<double>(-cnl::_impl::fractional_digits<fp>::value)}) +
            1);
    ...
    EXPECT_LE(
            std::abs(cnl::_impl::to_rep(exp2(maximum)) - cnl::_impl::to_rep(fp{exp2(doublerep)})),
            1)
