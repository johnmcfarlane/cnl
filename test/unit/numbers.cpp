...
#define CNL_EXPECT_NEAR(expected, actual, epsilon) \
    EXPECT_NEAR(double(expected), double(actual), double(epsilon))
            ...
            CNL_EXPECT_NEAR(
                    T{1} / std::sqrt(cnl::numbers::pi_v<T>), cnl::numbers::inv_sqrtpi_v<T>,
                    epsilon);
            ...
            CNL_EXPECT_NEAR(
                    0.5772156649015328606065120900824024L, cnl::numbers::egamma_v<T>, epsilon);
