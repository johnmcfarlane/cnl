...
static_assert(abs(scaled_integer<cnl::int64, power<>>(9223372036854775807))==9223372036854775807LL, "cnl::abs test failed");
static_assert(abs(scaled_integer<cnl::int64, power<>>(-9223372036854775807))==9223372036854775807LL, "cnl::abs test failed");
...
static_assert(identical(
        ...
        floor(cnl::scaled_integer<cnl::int32, power<-20>>(100.125))), "cnl::floor(scaled_integer)");
static_assert(identical(
        cnl::scaled_integer<cnl::int32, power<>>(-101),
        floor(cnl::scaled_integer<cnl::int32, power<-20>>(-100.125))), "cnl::floor(scaled_integer)");
static_assert(identical(
        cnl::scaled_integer<cnl::int32, power<1>>(-100),
        floor(cnl::scaled_integer<cnl::int32, power<1>>(-100))), "cnl::floor(scaled_integer)");

////////////////////////////////////////////////////////////////////////////////
// std specializations for 128-bit integer facilitate certain 64-bit operations

#if defined(CNL_INT128_ENABLED)
static_assert((scaled_integer<cnl::uint64, power<-8>>(1003006L)*scaled_integer<cnl::uint64, power<-8>>(7))==7021042UL, "cnl::scaled_integer test failed");
static_assert(static_cast<int>((scaled_integer<cnl::uint64, power<-8>>(65535)/scaled_integer<cnl::uint64, power<-8>>(256)))==255,
        "cnl::scaled_integer test failed");
static_assert(sqrt(scaled_integer<cnl::uint64, power<>>(9223372036854775807))==3037000499ULL, "cnl::sqrt test failed");
#endif
