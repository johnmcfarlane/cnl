...
namespace {  // NOLINT(cert-dcl59-cpp)
    ...
    namespace test_to_chars_natural {
        ...
        TEST(to_chars_natural, fixed_point)  // NOLINT
        {
    ...
    namespace test_to_chars {
        ...
        TEST(to_chars, fixed_point)  // NOLINT
        {
            test<22>("-5016.5091400146484375", cnl::fixed_point<int, -16>(-5016.5091400146484375));
        }

        TEST(to_chars, fixed_point_truncated)  // NOLINT
        {
            test<8>("5016.509", cnl::fixed_point<unsigned, -16>(5016.5091400146484375));
        }

        TEST(to_chars, fixed_point_just_big_enough)  // NOLINT
        {
            test<5>("-5016", cnl::fixed_point<short, -2>(-5016.5091400146484375));
        }

        TEST(to_chars, fixed_point_too_small)  // NOLINT
        {
            test<4>("", cnl::fixed_point<short, -2>(-5016.5091400146484375));
        }

        TEST(to_chars, fixed_point_small)  // NOLINT
        {
            test<7>("0.0039", cnl::fixed_point<int, -16>(0.00390625));
        }

        TEST(to_chars, fixed_point_small_single_digit)  // NOLINT
        {
            test<1>("0", cnl::fixed_point<int, -16>(0.00390625));
        }

        TEST(to_chars, fixed_point_small_double_digit)  // NOLINT
        {
            test<2>("0", cnl::fixed_point<int, -16>(0.00390625));
        }

        TEST(to_chars, fixed_point_small_tripple_digit)  // NOLINT
        {
            test<3>("0", cnl::fixed_point<int, -16>(0.00390625));
        }

        TEST(to_chars, fixed_point_small_negative_single_digit)  // NOLINT
        {
            test<1>("", cnl::fixed_point<int, -16>(-0.00390625));
        }

        TEST(to_chars, fixed_point_small_negative_double_digit)  // NOLINT
        {
            test<2>("-0", cnl::fixed_point<int, -16>(-0.00390625));
        }

        TEST(to_chars, fixed_point_small_negative_tripple_digit)  // NOLINT
        {
            test<3>("-0", cnl::fixed_point<int, -16>(-0.00390625));
        }

        TEST(to_chars, fixed_point_small_negative_quadruple_digit)  // NOLINT
        {
            test<4>("-0", cnl::fixed_point<int, -16>(-0.00390625));
        }

        TEST(to_chars, fixed_point_very_few_integer_digits)  // NOLINT
        {
            test<4>("-7", cnl::fixed_point<int, -28>(-7.00390625));
        }

        TEST(to_chars, fixed_point_decimal_positive)  // NOLINT
        {
            test<6>("17.917", cnl::fixed_point<int, -3, 10>(17.917));
        }

        TEST(to_chars, fixed_point_decimal_negative)  // NOLINT
        {
            test<5>("-5.25", cnl::fixed_point<int, -2, 10>(-5.25));
        }

        TEST(to_chars, fixed_point_decimal_no_fractional)  // NOLINT
        {
            test<7>("-517523", cnl::fixed_point<int, 0, 10>(-517523));
        }

        TEST(to_chars, fixed_point_octal_positive)  // NOLINT
        {
            test<9>("634124.25", cnl::fixed_point<int, -1, 8>(634124.25));
        }

        TEST(to_chars, fixed_point_octal_negative)  // NOLINT
        {
