...
namespace {
    ...
    namespace motivation_and_scope {
        ...
        namespace section_2_1 {
            constexpr auto positive_unsigned_overflow = UINT_MAX+1;  // 0U
            static_assert(identical(0U, positive_unsigned_overflow), "");

//            auto positive_signed_overflow = INT_MAX + 1; // undefined behavior

            constexpr auto negative_unsigned_overflow = 0U>-1;   // false
            static_assert(identical(false, negative_unsigned_overflow), "");
        }

        namespace section_2_2 {
            constexpr auto positive_unsigned_ok = static_cast<unsigned long long>(UINT_MAX)+1;
            static_assert(identical(0x100000000ULL, positive_unsigned_ok), "");

            constexpr auto positive_signed_ok = static_cast<long long>(INT_MAX)+1;
            static_assert(identical(0x80000000LL, positive_signed_ok), "");

            constexpr auto negative_unsigned_ok = static_cast<long long>(0U)>-1;
        ...
        namespace section_2_4 {
            constexpr auto square_root(elastic_integer<31> n)
            {
                return (n==2048000000) ? 45254.83399593904156165403917471 : throw std::exception();
            }

            constexpr auto d(elastic_integer<15> x, elastic_integer<15> y)  // [-32767..32767]
            {
        ...
        namespace section_2_5_a {
            constexpr auto a = elastic_integer(123);  // commonly deduced as elastic_integer<31>
            static_assert(identical(elastic_integer<31>{123}, a), "");

            constexpr auto b = elastic_integer(UINT64_C(4096));  // commonly deduced as elastic_integer<64, unsigned>
            static_assert(identical(elastic_integer<64, unsigned>{4096}, b), "");
        }

        namespace section_2_5_b {
            constexpr auto a = elastic_integer(constant<123>());  // deduced as elastic_integer<7>
            static_assert(identical(elastic_integer<7>{123}, a), "");

            constexpr auto b = elastic_integer(constant<4096>());  // deduced as elastic_integer<13>
        ...
        namespace section_2_5_c {
            ...
            constexpr auto a = elastic_integer(123_c);  // deduced as elastic_integer<7>
            static_assert(identical(elastic_integer<7>{123}, a), "");

            constexpr auto b = elastic_integer(4096_c);  // deduced as elastic_integer<13>
            static_assert(identical(elastic_integer<13>{4096}, b), "");
        }
        ...
        namespace section_4_1_2a {
            constexpr auto kibi = fixed_point<int32_t, -16>(1024); // 2^26
            static_assert(identical(1<<26, to_rep(kibi)), "");

//            constexpr auto mebi = kibi * kibi;   // fixed_point<int, -32>; value: 2^52
