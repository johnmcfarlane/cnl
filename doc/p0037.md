...
2. the `_multiply` and `_square` functions are not guaranteed to be
   available for 64-bit types;
3. the `_multiply` and `_square` functions produce undefined behavior
   when all input parameters are the *most negative number*;
4. the `_square` functions return an unsigned type;
5. the `_add`, `_subtract`, `_multiply` and `_divide` functions take
   heterogeneous `fixed_point` specializations;
6. the `_divide` and `_reciprocal` functions in no way guard against
   divide-by-zero errors;
7. the `trunc_shift_` functions return results of the same type as
   their first input parameter but with an adjusted `Exponent` value and
8. the list is by no means complete.
