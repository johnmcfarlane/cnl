...
1. If both arguments are fixed-point then:
   1. if
   then they are converted to
   then the inputs are adjusted as necessary to
...
fixed_point<uint8_t, -3>{8} + 3 === fixed_point<int64_t, -3>{11}
...
results in a value of 7.5. 
This is in contrast to equivalent integer results which underflow in the same circumstances.

Arithmetic operations using `fixed_point` return results with `Exponent` adjusted to minimize underflow.
However, division inevitable often loses precision 
