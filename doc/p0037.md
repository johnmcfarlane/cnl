**Document number**: LEWG, EWG, SG14, SG6: P0037R2_d03  
**Date**: 2016-02-11  
...
It adds three new class templates, `width<class>`, `set_width<class, int>` and `set_width_t<class, int>`, 
...
The value, `width<T>::value`, specifies the width of `T` in bits such that:
``` c++
width<int16_t>::value == 16
```

#### The `set_width` and `set_width_t` Helper Types

The compliment to `width` is `set_width`:
``` c++
template <class Archetype, unsigned MinNumBytes>
struct set_width;
```

An alias to `set_width` is `set_width_t` which can be defined as:
``` c++
template <class Archetype, unsigned MinNumBytes>
using set_width_t = typename set_width<Archetype, NumBytes>::type;
```

The resultant type is of the same signedness as `Archetype` 
and has a capacity of no less than `MinNumBits` bits 
such that for any integer, `T` and positive integer value, 'N':
``` c++
is_signed_v<set_width_t<T, N>> == is_signed_v<T>
```
and

``` c++
width<set_width_t<T, N>>::value >= N
```

are both true.

For example, the type of `set_width_t<unsigned, 8>` is `uint8_t`
and the type of `set_width_t<int8_t, 64>` is `int64_t`.
...
* adds new types, `set_width` and `set_width_t` to header, `type_traits`;
