...
They apply arithmetic operations to the underlying `Rep` value
without performing any scaling or widening - other than what occurs as a result of promotion.
...
auto p = multiply(f, f);
// p === fixed_point<int, -8>{254.00390625}
```
...
In contrast, the `*` operator returns wider results.
